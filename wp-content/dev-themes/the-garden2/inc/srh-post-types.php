...
// registration code for schedule post type
	...
	function register_exspenses_posttype() {
		$labels = array(
			'name' 				=> _x( 'Expenses', 'post type general name' ),
			'singular_name'		=> _x( 'Expense', 'post type singular name' ),
			'add_new' 			=> __( 'New Expense' ),
			'add_new_item' 		=> __( 'New Expense' ),
			...
			'not_found_in_trash'=> __( 'No Expenses found in the trash' ),
			'parent_item_colon' => __( '' ),
			'menu_name'			=> __( 'All Expenses' )
		);
		...
		$post_type_args = array(
			...
			'has_archive' 		=> true,
			'hierarchical' 		=> false,
			'rewrite' 			=> array('slug' => 'expenses', 'with_front' => false ),
			'supports' 			=> $supports,
			'menu_position' 	=> 5,
	...
	add_action('init', 'register_exspenses_posttype');// registration code for payments post type #TODO Typo. Change "exspenses" to "expenses"
...
// Taxonomy Export Code
		...
		// registration code for expense_category taxonomy
		function register_expense_category_tax() {
			$labels = array(
				'name' 					=> _x( 'Categories', 'taxonomy general name' ),
				'singular_name' 		=> _x( 'Expense Category', 'taxonomy singular name' ),
				'add_new' 				=> _x( 'New Expense Category', 'Expense Category'),
				'add_new_item' 			=> __( 'New Expense Category' ),
				'edit_item' 			=> __( 'Edit Expense Category' ),
				'new_item' 				=> __( 'New Expense Category' ),
				'view_item' 			=> __( 'View Expense Category' ),
				'search_items' 			=> __( 'Search Categories' ),
				'not_found' 			=> __( 'No Expense Category found' ),
				'not_found_in_trash' 	=> __( 'No Expense Category found in Trash' ),
			);
			
			$pages = array('exspenses');
			
			$args = array(
				'labels' 			=> $labels,
				'singular_label' 	=> __('Expense Category'),
				'public' 			=> true,
				'show_ui' 			=> true,
				'hierarchical' 		=> true,
				'show_tagcloud' 	=> false,
			...
			register_taxonomy('expense_category', $pages, $args);
		}
		add_action('init', 'register_expense_category_tax');
...
$expensedetails_12_metabox = array(
	'id' => 'expensedetails',
	'title' => 'Expense details',
	'page' => array('exspenses'),
	'context' => 'normal',
	'priority' => 'default',
	'fields' => array(
				...
				array(
					'name' 			=> 'Total Amount of Expense',
					'desc' 			=> '',
					'id' 				=> 'ecpt_total_amount_of_expense',
					'class' 			=> 'ecpt_total_amount_of_expense',
					'type' 			=> 'text',
					'rich_editor' 	=> 0,
					'max' 			=> 0				),
			
				array(
					'name' 			=> 'Expense Partial Amount Spent',
					'desc' 			=> 'Collaboratively administrate empowered markets via plug-and-play networks. Dynamically procrastinate B2C users after installed base benefits. Dramatically visualize customer directed convergence without revolutionary ROI. Efficiently unleash cross-media information without cross-media value. Quickly maximize timely deliverables for real-time schemas. Dramatically maintain clicks-and-mortar solutions without functional solutions.',
					'id' 				=> 'ecpt_expense_partial_amount_spent',
					'class' 			=> 'ecpt_expense_partial_amount_spent',
					'type' 			=> 'text',
					'rich_editor' 	=> 1,
					'max' 			=> 0				),
				)
...
add_action('admin_menu', 'ecpt_add_expensedetails_12_meta_box');
function ecpt_add_expensedetails_12_meta_box() {

	global $expensedetails_12_metabox;

	foreach($expensedetails_12_metabox['page'] as $page) {
		add_meta_box($expensedetails_12_metabox['id'], $expensedetails_12_metabox['title'], 'ecpt_show_expensedetails_12_box', $page, 'normal', 'default', $expensedetails_12_metabox);
	}
}

// function to show meta boxes
function ecpt_show_expensedetails_12_box()	{
	global $post;
	global $expensedetails_12_metabox;
	global $ecpt_prefix;
	global $wp_version;
	...
	// Use nonce for verification
	echo '<input type="hidden" name="ecpt_expensedetails_12_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';

	echo '<table class="form-table">';

	foreach ($expensedetails_12_metabox['fields'] as $field) {
		// get current post meta data
...
// Save data from meta box
add_action('save_post', 'ecpt_expensedetails_12_save');
function ecpt_expensedetails_12_save($post_id) {
	global $post;
	global $expensedetails_12_metabox;

	// verify nonce
	if ( ! isset( $_POST['ecpt_expensedetails_12_meta_box_nonce'] ) || ! wp_verify_nonce($_POST['ecpt_expensedetails_12_meta_box_nonce'], basename(__FILE__))) {
		return $post_id;
	}
