...
require_once ($includes_path . 'theme-functions.php'); 		// Register Custom Wigetized Areas
...
function get_all_journaling_posts( $query ) {
    if( !is_admin() && $query->is_main_query() && is_post_type_archive( 'journaling' ) ) {
        $query->set( 'posts_per_page', '-1' );
    }
}
add_action( 'pre_get_posts', 'get_all_journaling_posts' );
